...
public class CacheMethods {
    ...
    public void initTeams( Player player ) {
        ...
        this.serverSection.getConfigurationSection( "tablist.prefix" )
                .getKeys( false )
                .stream()
                .map( permissionName -> this.serverSection.getStringList( "tablist.prefix." + permissionName ) )
                .forEach( permissionNameValues -> {
                    if ( user.isInGuild() ) {
                        String guildSuffix = permissionNameValues.get( 2 ).replace( "{GUILD}", Guild.getPlugin().getGuildController().sendGuildName( player ) );
